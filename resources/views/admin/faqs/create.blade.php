...
@section('title', settings()->website_title . ' | ' . __('words.create_faq'))
@section('breadcrumb')
    <div class="d-flex align-items-baseline flex-wrap mr-5">
        ...
        <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
            ...
            <li class="breadcrumb-item">
                <span class="text-muted">{{ __('words.create_faq') }}</span>
            </li>
        </ul>
        <!--end::Breadcrumb-->
    </div>
    ...
    <div class="card card-custom mb-2">
        <div class="card-header card-header-tabs-line">
            <div class="card-title">
                <h3 class="card-label">{{ __('words.create_faq') }}</h3>
            </div>
                    ...
                    <div class="tab-pane fade show @if ($key == 0) active @endif" id="{{ $locale }}"
                        ...
                        <div class="col form-group">
                            ...
                            <textarea class="form-control ckeditor @error($locale . '.answer') is-invalid @enderror " type="text"
                                name="{{ $locale . '[answer]' }}" rows="4">{{ old($locale . '.answer') }} </textarea>
                            @error($locale . '[answer]')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
